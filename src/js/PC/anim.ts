...
export const SearchDevice = (search: string) => {
    ...
    //if the space at the end is missing, add it
    ...
    if (search.includes("typ:")) {
        //get the value from after the typ: till the next space
        const value = search.substring(search.indexOf("typ:") + 4, search.indexOf(" ", search.indexOf("typ:")));
        if (value.length != 0) attributes.push({ attribute: "typ", value: value });
    }
    if (search.includes("her:")) {
        //get the value from after the her: till the next space
    ...
    if (search.includes("kom:")) {
        //get the value from after the kom: till the next space, except the value starts and ends with ", then escape the space

        let value = search.substring(search.indexOf("kom:") + 4, search.indexOf(" ", search.indexOf("kom:")));
        if (value.startsWith('"')) {
            //Now we need to find the end of the string
            let end = value.indexOf('"', 1);
            if (end == -1) return ShowError("ParseError: Missing \" at the end");
            value = value.slice(1, end);
            attributes.push({ attribute: "kom", value: value });
        }
    ...
    if (attributes.length == 0) {
        //if no attributes are given, filter all devices by the search string
        devices.forEach(device => {
            if (device.it_nr.includes(search)) result.push(device);
            if (device.kommentar?.includes(search)) result.push(device);
        });
        ClearTable();
